...
async def test_retries_on_5xx_then_succeeds():
    ...
    async with FedstackClient("test", max_retries=3, min_interval=0) as client:
...
async def test_gives_up_after_max_retries_with_a_readable_error():
    ...
    async with FedstackClient("grants.gov", max_retries=2, min_interval=0) as client:
...
async def test_honours_retry_after_header(monkeypatch):
    ...
    monkeypatch.setattr("fedstack.http.asyncio.sleep", fake_sleep)
    respx.get(URL).mock(
        side_effect=[
    ...
    async with FedstackClient("test", max_retries=2, min_interval=0) as client:
        await client.get_json(URL)
...
async def test_non_json_body_raises_upstream_error_not_a_json_traceback():
    ...
    async with FedstackClient("test", max_retries=0, min_interval=0) as client:
...
async def test_get_json_drops_none_params():
    ...
    async with FedstackClient("test", min_interval=0) as client:
